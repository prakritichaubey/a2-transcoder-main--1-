...
def download_S3_file(bucket_name: str, object_key: str) -> Path:
    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
    temp_file.close()  # Close so boto3 can write
...
def _run_job(job_id: int) -> None:
    ...
    in_path = None
    try:
        in_path = download_S3_file(bucket_name, f"incoming/{video.filename}")
    except Exception as e:
        job.status = "failed"
        job.error = f"Failed to download from S3: {str(e)}"
        db.commit()
        return
    out_dir = OUTPUTS_DIR / f"job_{job.id}"
    out_dir.mkdir(parents=True, exist_ok=True)
